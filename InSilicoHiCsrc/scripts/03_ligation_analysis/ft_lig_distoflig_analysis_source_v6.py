...
# Template script for ligation analysis stage of the in-silico Hi-C pipeline.
#
# Placeholder variables beginning with "@" are replaced by the generator
# shell script to produce per-trajectory analysis scripts.
#
# This script analyzes MD trajectories and associated metadata to identify
# ligation events between DNA fragment ends under different ligation probabilities.
...
fn_base = '@fn_base'
rc = @rc  # spatial cutoff for ligation (distance threshold)
...
if not os.path.exists(contactmap_file):
    ...
    # Iterate over trajectory frames and perform ligation experiments
    # simultaneously for all ligation probabilities (lig_probs).
